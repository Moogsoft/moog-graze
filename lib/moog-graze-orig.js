...
function getGrazeLogin(callback) {
    ...
    var grazeLoginRequest = https.request(grazeLoginOpts, (res) => {
        var tokenData={};
        var returnString="";
 ...
 * var grazeREST = new moog.grazeREST(options);
 *
 * @requires url
 * @requires util
 * @requires events
...
var debug = function(){
    ...
    // TODO: Change for another logging system, utils.log is deprecated
    //
    util.log('[moog-graze.js] ' + callee + ' -> ' + args);
};
 ...
 *     'https://moogserver:8080'.
 * @property {string} [auth_token='xxxxxxxx'] - the auth token returned by the auth endpoint.
 ...
 * @property {string} [authUser] - User name for authentication endpoint
 * @property {string} [authPass] - Password for authentication endpoint
 *
 * @typedef {object} requesttHeaders
 * @description The https headers.
 * @type {object}
 ...
 * @property {number} port=8080 - from the url or 8080.
 * @property {string} [method='POST'] - GET or POST.
 */
 ...
 * @constructor
 */
exports.grazeREST = function grazeREST(options) {
    'use strict';
    if ((this instanceof grazeREST)){
        util.log("ERROR Don't use new for grazeREST");
        return;
    }
    var that = this;
    var proto = {};
...
/**
 * Add the Event emitter to grazeREST
 */
util.inherits(exports.grazeREST, events.EventEmitter);
